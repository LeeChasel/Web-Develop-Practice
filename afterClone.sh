...
sed -e 's/DB_HOST=127.0.0.1/DB_HOST=mysql/g' -e 's/DB_DATABASE=laravel/DB_DATABASE=userinfos/g' -e 's/DB_PASSWORD=/DB_PASSWORD=root/g' -e 's/WORKSPACE_TIMEZONE=UTC/WORKSPACE_TIMEZONE=Asia\/Taipei/g' Laravel/Laravel/.env.example > Laravel/Laravel/.env
